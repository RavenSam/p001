...
export default function Navbar() {
    ...
    return (
        <nav
            ref={navRef}
            className={`fixed opacity-0 top-6 left-1/2 -translate-x-1/2 w-[90%] max-w-6xl rounded-full p-3 flex items-center justify-between z-50 transition-all duration-500 ${isScrolled ? "glass-panel backdrop-blur-sm shadow-2xl px-10" : "bg-transparent border border-transparent"
                }`}
        >
            <div className="flex items-center gap-2">
                <Link href="/" className="font-serif text-xl tracking-wide text-rubis shrink-0">
                    MSDI
                </Link>
            </div>
